...
def batchnorm_backward(dout, cache):
    ...
    dx_hat = dout * gamma
    dvar = np.sum(dx_hat * (x-sample_mean)*(-1/2)*(sample_var + eps)**(-1.5), axis=0)
    dmean = np.sum(dx_hat * -1 / (np.sqrt(sample_var + eps)), axis =0) + dvar * np.sum(-2 *(x - sample_mean)) / N
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    H_out = int(1+  ((np.shape(x)[2] - np.shape(w)[2] + 
        2*conv_param['pad'])/(conv_param['stride']))) 
    W_out = int(1 + ((np.shape(x)[3] - np.shape(w)[3] + 2*conv_param['pad'])/
        (conv_param['stride']))) 
    ...
    for n in range(N): # Iterate over all the inputs
        for f in range(F): # Iterate over all the filters
            for h in range(0, H_out):
                for w_ in range(0, W_out):
                    ...
                    out[n, f, h, w_] = np.sum(x_slice * w[f, :, :, :]) + b[f]
...
def conv_backward_naive(dout, cache):
    ...
    # Add padding around the input and gradient
        ...
        for c in range(C):
            for h in range(H_out):
                for w in range(W_out):
                    ...
                    # Find the max pooling window
                    x_pooling_window = x[n, c, h_start:h_end, w_start:w_end]
                    # Mask to identify the max element in the window
                    max_mask = x_pooling_window == np.max(x_pooling_window)
                    
                    # Only the gradient for the max element is non-zero
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N,C,H,W = x.shape
    #need x : (N,D) D=C and N = NxHxW so put in (N,H,W,C) then reshape
    ...
    #change out from (N*H*W,C) to (N, C, H, W)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    gn_param["mode"] = gn_param.get("mode", "train")   
    N,C,H,W = x.shape  
    assert C % G == 0, "G is not a divisor of C"
    x_group = x.reshape(N * G, C // G * H * W).T
    group_mean = np.mean(x_group, axis = 0)
    group_var = np.var(x_group, axis = 0)
    group_x_hat = (x_group - group_mean) / np.sqrt(group_var + eps)
    x_hat = group_x_hat.T.reshape(N, C, H, W)
    out = gamma * x_hat + beta
    cache = (G, x_group, x_hat, gamma, beta, group_mean, group_var, eps)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    G, x_group, x_hat, gamma, beta, group_mean, group_var, eps = cache
    N, C, H, W = dout.shape
    ...
    dx_hat = dout * gamma
    dx_hat_group = dx_hat.reshape(N * G, C // G * H * W).T

    dvar = np.sum(dx_hat_group * (x_group-group_mean)*(-1/2)*(group_var + eps)**(-1.5), axis=0)
    dmean = np.sum(dx_hat_group * -1 / (np.sqrt(group_var + eps)), axis =0) + dvar * np.sum(-2 *(x_group - group_mean)) / N_group
    dx_group = dx_hat_group * 1 / np.sqrt(group_var + eps) + (dvar * 2 * (x_group - group_mean)) / N_group + (dmean / N_group)

    dx = dx_group.T.reshape(N, C, H, W)
